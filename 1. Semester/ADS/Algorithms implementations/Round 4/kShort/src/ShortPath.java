...
public class ShortPath {
    public int shortestPath(Edge[] edges, int nodeCount, int maxEdges, int sourceNode, int targetNode) {
        // dp[i][v] = shortest distance to reach vertex v using at most i edges
        int[][] dp = new int[maxEdges + 1][nodeCount];
        ...
        // Distance to sourceNode using 0 edges is 0
        dp[0][sourceNode] = 0;

        // Loop over edge counts from 1 to maxEdges
        for (int i = 1; i <= maxEdges; i++) {
            // Copy previous distances (using at most i-1 edges)
            ...
            for (Edge e : edges) {
                ...
                // Relax if using edge (u->v) gives shorter distance
        ...
        // If target is unreachable, return Integer.MAX_VALUE
        return dp[maxEdges][targetNode] == Integer.MAX_VALUE ? Integer.MAX_VALUE : dp[maxEdges][targetNode];
    }
