...
public class Closest {
    public int computeClosest(ArrayList<Integer> input) {
        Collections.sort(input); // Sort the list, so that we can compare adjacent elements only
        int minDiff = Integer.MAX_VALUE; // Start with the largest possible difference

        for (int i = 1; i < input.size(); i++) {
            int diff = input.get(i) - input.get(i-1); // Difference between adjacent elements
            if (diff < minDiff) { // Check if this is the smallest difference found so far
                minDiff = diff;
                if(minDiff == 0) break; // Can't get smaller than 0, so break early
            }
        }
