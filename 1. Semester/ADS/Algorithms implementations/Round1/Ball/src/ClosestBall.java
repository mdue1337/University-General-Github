...
public class ClosestBall {
    public int computeClosest(ArrayList<Integer> players, ArrayList<Integer> balls) {
        ...
        int j = 0; // Pointer for balls
        int i = 0; // Pointer for players

        while(i < players.size() && j < balls.size()) {
            int playerPos = players.get(i);
            int ballPos = balls.get(j);
            int currentDiff = Math.abs(playerPos - ballPos);
            minDiff = Math.min(minDiff, currentDiff);

            // Move the pointer that points to the smaller value, because array is sorted.
